...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(l):
        ...
        if (dsort[0] / dsort[1]) <= threshold:
            minimum = np.argmin(dtemp)
            matches.append([i ,minimum])
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        maxT = np.zeros(N, dtype=np.int32) #store max inliers
        nT = 0 
        index = np.random.choice(N, n_samples, replace=False) #random index set
        t1 = matched1[index,:]
        t2 = matched2[index,:]
        H = np.linalg.lstsq(t2, t1, rcond=None)[0] #affine matrix calc
        H[:, 2] = np.array([0, 0, 1])
        
        #Euclidean Norm + find inliers
        maxT = np.linalg.norm(matched2.dot(H) - matched1, axis=1)**2<threshold
        nT = np.sum(maxT)

        if nT > n_inliers: #store max set of inliers
            max_inliers = maxT.copy()
            n_inliers = nT

    # estimation for least squares on inliers
    H = np.linalg.lstsq(matched2[max_inliers], matched1[max_inliers], rcond=None)
    H = H[0]
    H[:,2] = np.array([0, 0, 1])
        ...
        for j in range(cols):
            for k in range(pixels_per_cell[0]):
                for l in range(pixels_per_cell[1]):
                    distance_lower = theta_cells[i][j][k][l] % degrees_per_bin
                    divR = float(distance_lower) / degrees_per_bin
                    lower_num = (int(np.trunc(theta_cells[i][j][k][l] / degrees_per_bin))) % 9

                    upper_num = (lower_num + 1) % 9
                    cells[i][j][upper_num] += (divR*G_cells[i][j][k][l])
                    cells[i][j][lower_num] += ((1-divR) * G_cells[i][j][k][l])
...
def linear_blend(img1_warped, img2_warped):
    ...
    w = np.zeros(out_W)
    w[0:left_margin] = 1
    w[left_margin:right_margin] = np.linspace(1,0,num=(right_margin - left_margin))
    merged = img1_warped * img1_mask * w + img2_warped * img2_mask * (1-w)
